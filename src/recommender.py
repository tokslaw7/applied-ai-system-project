...
def recommend_songs(user_prefs: Dict[str, any], songs: List[Song], k: int = 10) -> List[Tuple]:
    """
    Top-level function to generate song recommendations.
    
    Args:
        user_prefs: Dictionary with user preferences (genre, mood, energy, etc.)
    ...
    """
    # Build UserProfile from user_prefs dict
    user_profile = UserProfile(
        ...
        activity_context=user_prefs.get("activity_context", "general"),
        time_of_day=user_prefs.get("time_of_day", "any"),
        explicit_tolerance=user_prefs.get("explicit_tolerance", 1.0),
    )
    ...
    for song, score in recommendations:
        explanation_obj = recommender.explain_recommendation(user_profile, song)
        song_dict = {
        ...
        results.append((song_dict, score, explanation_obj["explanation"]))
